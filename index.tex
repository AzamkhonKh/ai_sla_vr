...
\begin{abstract}
Integrating Large Language Models into Virtual Reality provides a transformative environment for language acquisition, yet combining these technologies introduces critical performance and economic constraints. Current cloud-based systems suffer from high latency that breaks immersion and become prohibitively expensive when scaling to thousands of concurrent users. To address this, we propose MundoVR, a hybrid architecture that strategically splits processing tasks between the headset and the cloud to enable real-time, cost-effective conversations. This work contributes a formally specified, scalable system blueprint that balances technical efficiency with pedagogical effectiveness, offering a clear path for future immersive educational tools.
\end{abstract}
...
The most critical constraint is the Motion-to-Photon Latency required for VR immersion. Industry standards establish that visual rendering must occur within 20\,ms to prevent simulator sickness and maintain presence \cite{jerald2015vr}. Conversational interactions require a substantially looser but still strict latency budget to maintain conversational continuity and preserve the sense of presence. We establish a research-informed design target of 1.5\,s (p99) for total conversational latency, based on real-world performance characteristics of existing conversational systems and published AI inference benchmarks. This target is decomposed into component-level budgets based on typical processing times reported in speech and language processing literature: Network round-trip time (RTT, 100\,ms), Speech-to-Text (STT) Processing (300\,ms), Large Language Model (LLM) Inference (800\,ms), Text-to-Speech (TTS) Synthesis (200\,ms), and Client Buffer (100\,ms). Since standard cloud LLM application programming interfaces (APIs) typically exhibit latencies of 2--5\,s, the architecture must be designed around optimized, self-hosted models or specialized inference engines to achieve the 800\,ms inference target.
