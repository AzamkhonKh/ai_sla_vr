...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{figures/01_requirements/usecase.png}
\caption{SysML Use Case Diagram: Interaction flows for key use cases. Shows relationships (include, extend) between Practice Conversation, Analyze Pronunciation, Adjust Difficulty, Select Scenario, and Track Progress.}
\label{fig:usecase-interactions}
\end{figure}

\subsection*{Requirements Specification}
This section groups requirements by primary Use Cases, illustrating how functional capabilities are derived from user goals and how non-functional quality attributes constrain these interactions.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{figures/01_requirements/requirenments.png}
\caption{SysML Requirements Diagram: Requirements hierarchy grouped by primary capabilities. (1) Real-Time Immersive Conversation derives Speech Processing (FR1), Dialogue Generation (FR2), and Multimodal Synthesis (FR3), constrained by Performance (NF1) and Reliability (NF2). (2) Adaptive Learning \& Feedback derives Pronunciation Analysis (FR4), Adaptive Progression (FR7), and Learning Support (FR5), satisfying Usability (NF3). (3) System Access \& Progression Tracking derives Persistence (FR7) and Identity Management (FR8), constrained by Scalability (NFR5), Security (NFR6), and Recoverability (NFR7).}
\label{fig:requirements}
\end{figure}

\subsubsection*{UC1: Real-Time Immersive Conversation}
\textit{Description: The learner engages in spontaneous spoken dialogue with a virtual avatar in a specific VR scenario (e.g., ordering coffee).}

\begin{itemize}[leftmargin=*]
    \item \textbf{Derived Functional Requirements:}
    \begin{itemize}
        \item \textbf{FR1 (Speech Input):} The system shall capture user audio streams and transcribe speech to text.
        \item \textbf{FR2 (Dialogue Generation):} The system shall generate contextual text responses based on the transcribed input and current scenario state.
        \item \textbf{FR3 (Multimodal Output):} The system shall synthesize audible speech from text and map phonemes to avatar visemes (lip movements).
    \end{itemize}
    
    \item \textbf{Non-Functional Constraints:}
    \begin{itemize}
        \item \textbf{NFR1 (Performance - Latency):} 
            \begin{itemize}
                \item \textit{Immersion Loop:} Viseme/Audio synchronization delay $<$ 200ms.
                \item \textit{Dialogue Loop:} Total round-trip time (End-of-Speech to Audio-Start) $<$ 1.5 seconds.
            \end{itemize}
        \item \textbf{NFR2 (Reliability - Accuracy):} Speech-to-Text Word Error Rate (WER) shall be $<$ 10\% for native-accented speech.
    \end{itemize}
\end{itemize}

\subsubsection*{UC2: Adaptive Learning \& Feedback}
\textit{Description: The learner receives immediate corrective feedback and the system adjusts the scenario difficulty based on performance.}

\begin{itemize}[leftmargin=*]
    \item \textbf{Derived Functional Requirements:}
    \begin{itemize}
        \item \textbf{FR4 (Pronunciation Analysis):} The system shall compare user audio against native speaker models to identify phonemic errors.
        \item \textbf{FR5 (Adaptive Progression):} The system shall dynamically adjust vocabulary complexity and speaking rate based on the user's proficiency level.
        \item \textbf{FR6 (Learning Support):} The system shall provide on-demand auxiliary aids (e.g., translations, rephrasing suggestions) within the VR HUD.
    \end{itemize}
    
    \item \textbf{Non-Functional Constraints:}
    \begin{itemize}
        \item \textbf{NFR3 (Usability - Feedback):} Corrective feedback must be presented visually within the VR field of view without occluding the avatar.
        \item \textbf{NFR4 (Functional Suitability):} Generated dialogue must adhere to CEFR (Common European Framework of Reference) level constraints defined in the user profile.
    \end{itemize}
\end{itemize}

\subsubsection*{UC3: System Access \& Progression Tracking}
\textit{Description: The learner logs in, resumes sessions, and tracks long-term progress across different devices.}

\begin{itemize}[leftmargin=*]
    \item \textbf{Derived Functional Requirements:}
    \begin{itemize}
        \item \textbf{FR7 (Persistence):} The system shall record and store user session data, including learned vocabulary and completed scenarios.
        \item \textbf{FR8 (Identity Management):} The system shall authenticate users and load their specific learning profile upon entry.
    \end{itemize}
    
    \item \textbf{Non-Functional Constraints:}
    \begin{itemize}
        \item \textbf{NFR5 (Scalability):} The backend shall support 10,000 concurrent active sessions with automatic horizontal scaling.
        \item \textbf{NFR6 (Security):} All user voice data and PII must be encrypted at rest (AES-256) and in transit (TLS 1.3).
        \item \textbf{NFR7 (Recoverability):} User progress data shall be retained for a minimum of 12 months of inactivity.
    \end{itemize}
\end{itemize}

\begin{table}[H]
\centering
\caption{Computational Resource and Scaling Analysis per Interaction Turn}
\label{tab:resource-analysis}
\renewcommand{\arraystretch}{1.3}
\small
\begin{tabularx}{\textwidth}{@{}l l X l@{}}
\toprule
\textbf{Domain} & \textbf{Component} & \textbf{Computation (Single Iteration)} & \textbf{Scaling Model} \\
\midrule
\textbf{On-Device} & VR Client & Audio Capture, VAD, Rendering ($<$50ms) & 1:1 (Per User) \\
\midrule
\textbf{Cloud} & API Gateway & Auth, Routing, Rate Limiting ($<$10ms) & Stateless Horizontal \\
(Backend) & Orchestration & Context Retrieval, State Mgmt ($<$50ms) & Event-Driven \\
\midrule
\textbf{Cloud} & STT (Whisper) & Audio Stream Decoding (200-500ms) & Dynamic Batching \\
(AI Compute) & LLM (Qwen/Mistral) & Context Processing + Token Gen (800-1200ms) & Queue-based HPA \\
 & TTS (Piper) & Phoneme Synthesis + Audio Gen (200-300ms) & On-Demand \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth,height=0.6\textheight,keepaspectratio]{figures/03_behavior/sysml-act-02-dialogue-simple.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{figures/03_behavior/sysml-seq-01-conversation.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{figures/03_behavior/sysml-seq-02-conversation.png}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Primary:} Qwen2.5 72B (Alibaba, Apache 2.0) --- 128K context window, INT4-AWQ quantization, $4\times$ A100 GPUs (40GB VRAM each), 850-1100ms inference latency, multilingual support (EN/DE/ES/ZH)
    \item \textbf{Fallback:} Mistral Large 2 123B (Mistral AI, Apache 2.0) --- 128K context window, INT8 quantization, $8\times$ A100 GPUs (80GB VRAM each), 1400-1800ms inference latency, superior reasoning quality
    \item \textbf{Alternative:} Llama 3.1 70B (Meta, Llama 3.1 License) --- 128K context window, INT8 quantization, $4\times$ A100 GPUs (40GB VRAM each), 900-1200ms inference latency
\end{itemize}
...
\begin{tabularx}{\textwidth}{@{}l l X l@{}}
\toprule
\textbf{Layer} & \textbf{Component} & \textbf{Configuration \& Specs} & \textbf{Scaling Strategy} \\
\midrule
...
\midrule
\textbf{Storage} & PostgreSQL & 2TB NVMe, Primary+Replica & Read Replicas \\
...
\caption{SysML Parametric Diagram (Level 1): System-wide constraints in three categories: (1) Performance (p99 latency $\leq$1.5s: on-device $\leq$200ms + network 50-100ms + backend 1.0-1.3s), (2) Hardware ($12\times$A100 GPUs: $4\times$40GB Qwen2.5, $8\times$80GB Mistral, $2\times$40GB Whisper, $1\times$40GB TTS; $6\times$EPYC 7763 CPUs: 384 cores, 3TB RAM; 4TB NVMe: 500K IOPS, 7 GB/s), (3) Resources (10K users, 99.5\% uptime). Dependencies: latency $f$(GPU compute, model size), throughput $f$(CPU cores, memory), data $f$(storage IOPS, cache hit).}
...
\caption{SysML Parametric Diagram (Level 2): Compute resource allocation. Primary LLM (Qwen2.5 72B INT4-AWQ): $4\times$A100 40GB, 36GB VRAM, 85 tokens/s, batch 32, 850-1100ms. Fallback LLM (Mistral Large 2 123B INT8): $8\times$A100 80GB, 80GB VRAM, 52 tokens/s, batch 16, 1400-1800ms. STT (Whisper): $2\times$A100 40GB, 200-500ms, 100 req/s. TTS (Piper): $1\times$A100 40GB, 200-300ms, 50 req/s. Total: $12\times$A100, 624GB VRAM (480GB LLM weights+cache, 80GB STT/TTS, 64GB activations), 3TB system RAM.}
...
\caption{SysML Parametric Diagram (Level 3): On-premises hardware specifications. LLM Models: Qwen2.5 72B INT4-AWQ ($4\times$A100 40GB, 36GB VRAM, 85 tokens/s, 850-1100ms, 128K context), Mistral Large 2 123B INT8 ($8\times$A100 80GB, 80GB VRAM, 52 tokens/s, 1400-1800ms, 128K context), Llama 3.1 70B ($4\times$A100 40GB, 35GB VRAM, 70 tokens/s, 900-1200ms, 128K context). Hardware: AMD EPYC 7763 (64 cores @ 2.45/3.5GHz, 512GB DDR4-3200 @ 204GB/s), NVIDIA A100 (40/80GB HBM2e @ 1.6TB/s, NVLink 600GB/s, PCIe Gen4 64GB/s). Environment: liquid cooling 45°C max, 42U rack, 10Gbps uplink. Performance: Qwen2.5 batch 32 at 85 tokens/s with 36GB+40GB cache; Mistral batch 16 at 52 tokens/s with 80GB+60GB cache. Total bandwidth: 2TB/s aggregate. Dependencies: Model $\Rightarrow$ GPU count, VRAM $\Rightarrow$ batch size, Context $\Rightarrow$ cache size.}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Disadvantages:}
        \begin{itemize}
            ...
            \item Single point of failure: Any cloud outage immediately affects all users.
